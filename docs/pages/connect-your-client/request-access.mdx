...
Teleport users can request access to a resource by running the following
command:
...
```code
$ tsh request create --resource /teleport.example.com/namespace/mycluster/prod
```
...
```code
$ tsh request create --resource /teleport.example.com/namespace/mycluster/^dev-[a-z0-9-]+$
```
