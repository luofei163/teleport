...
func (r *Manager) ResolveRecordingEncryption(ctx context.Context) (*recordingencryptionv1.RecordingEncryption, error) {
...
func (r *Manager) searchActiveKeys(ctx context.Context, activeKeys []*recordingencryptionv1.WrappedKey, publicKey []byte) (*types.EncryptionKeyPair, error) {
	for _, key := range activeKeys {
...
func (r *Manager) FindDecryptionKey(ctx context.Context, publicKeys ...[]byte) (*types.EncryptionKeyPair, error) {
	encryption, err := r.GetRecordingEncryption(ctx)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	if len(publicKeys) == 0 {
		return r.searchActiveKeys(ctx, activeKeys, nil)
	}

	for _, publicKey := range publicKeys {
		found, err := r.searchActiveKeys(ctx, activeKeys, publicKey)
		if err != nil {
			if trace.IsNotFound(err) {
				continue
			}
