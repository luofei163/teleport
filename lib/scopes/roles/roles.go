...
func WeakValidatedAssignableScopes(role *srpb.ScopedRole) iter.Seq[string] {
	return func(yield func(string) bool) {
		for _, assignableScope := range role.Spec.AssignableScopes {
			if err := scopes.WeakValidateGlob(assignableScope); err != nil {
				// ignore invalid assignable scopes
				continue
			}

			if !scopes.Glob(assignableScope).IsSubjectToPolicyResourceScope(role.Scope) {
...
func WeakValidateRole(role *srpb.ScopedRole) error {
	...
	if err := scopes.WeakValidate(role.Scope); err != nil {
		return trace.BadParameter("scoped role %q has invalid scope: %v", role.Metadata.Name, err)
	}
...
func StrongValidateRole(role *srpb.ScopedRole) error {
	...
	if err := validateRoleName(role.Metadata.Name); err != nil {
		return trace.BadParameter("scoped role name %q does not conform to segment naming rules: %v", role.Metadata.Name, err)
	}

	if err := scopes.StrongValidate(role.Scope); err != nil {
		return trace.BadParameter("scoped role %q has invalid scope: %v", role.Metadata.Name, err)
	}

	if len(role.Spec.AssignableScopes) == 0 {
		return trace.BadParameter("scoped role %q does not have any assignable scopes", role.Metadata.Name)
	}

	if len(role.Spec.AssignableScopes) > maxAssignableScopes {
		return trace.BadParameter("scoped role %q has too many assignable scopes (max %d)", role.Metadata.Name, maxAssignableScopes)
	}

	for _, scopeGlob := range role.Spec.AssignableScopes {
		if err := scopes.StrongValidateGlob(scopeGlob); err != nil {
			return trace.BadParameter("scoped role %q has invalid assignable scope %q: %v", role.Metadata.Name, scopeGlob, err)
		}

		if !scopes.Glob(scopeGlob).IsSubjectToPolicyResourceScope(role.Scope) {
			return trace.BadParameter("scoped role %q has assignable scope %q that is not a sub-scope of the role's scope %q", role.Metadata.Name, scopeGlob, role.Scope)
		}
	}
...
func validateRoleName(name string) error {
	...
	return trace.Wrap(scopes.ValidateSegment(name))
}
...
func commonValidateRole(role *srpb.ScopedRole) error {
	if role.Metadata == nil {
		return trace.BadParameter("scoped role is missing metadata")
	}

	if role.Metadata.Name == "" {
		return trace.BadParameter("scoped role is missing name")
	}

	if role.Kind == "" {
		return trace.BadParameter("scoped role %q is missing kind", role.Metadata.Name)
	}

	if role.Kind != KindScopedRole {
		return trace.BadParameter("scoped role %q has invalid kind %q, expected %q", role.Metadata.Name, role.Kind, KindScopedRole)
	}

	if role.SubKind != "" {
		return trace.BadParameter("scoped role %q has unknown sub_kind %q", role.Metadata.Name, role.SubKind)
	}

	if role.Version == "" {
		return trace.BadParameter("scoped role %q is missing version", role.Metadata.Name)
	}

	if role.Version != types.V1 {
		return trace.BadParameter("scoped role %q has unsupported version %q (expected %q)", role.Metadata.Name, role.Version, types.V1)
	}

	if role.Scope == "" {
		return trace.BadParameter("scoped role %q is missing scope", role.Metadata.Name)
	}

	if role.Spec == nil {
		return trace.BadParameter("scoped role %q is missing spec", role.Metadata.Name)
	}
...
func WeakValidatedSubAssignments(assignment *srpb.ScopedRoleAssignment) iter.Seq[*srpb.Assignment] {
	return func(yield func(*srpb.Assignment) bool) {
		for _, subAssignment := range assignment.Spec.Assignments {
			if subAssignment.Role == "" {
				// ignore sub-assignments with missing role
				continue
			}

			if err := scopes.WeakValidate(subAssignment.Scope); err != nil {
...
func WeakValidateAssignment(assignment *srpb.ScopedRoleAssignment) error {
	...
	if err := scopes.WeakValidate(assignment.Scope); err != nil {
		return trace.BadParameter("scoped role assignment %q has invalid scope: %v", assignment.Metadata.Name, err)
	}
...
func StrongValidateAssignment(assignment *srpb.ScopedRoleAssignment) error {
	...
	if _, err := uuid.Parse(assignment.Metadata.Name); err != nil {
		return trace.BadParameter("scoped role assignment %q has invalid name (must be uuid): %v", assignment.Metadata.Name, err)
	}

	if err := scopes.StrongValidate(assignment.Scope); err != nil {
		return trace.BadParameter("scoped role assignment %q has invalid scope: %v", assignment.Metadata.Name, err)
	}

	if len(assignment.Spec.Assignments) == 0 {
		return trace.BadParameter("scoped role assignment %q does not assign any roles", assignment.Metadata.Name)
	}

	if len(assignment.Spec.Assignments) > MaxRolesPerAssignment {
		return trace.BadParameter("scoped role assignment %q contains too many sub-assignments (max %d)", assignment.Metadata.Name, MaxRolesPerAssignment)
	}

	for i, subAssignment := range assignment.Spec.Assignments {
		if subAssignment.Role == "" {
			return trace.BadParameter("scoped role assignment %q is missing role in sub-assignment %d", assignment.Metadata.Name, i)
		}

		if err := validateRoleName(subAssignment.Role); err != nil {
			return trace.BadParameter("scoped role assignment %q has invalid role name in sub-assignment %d: %v", assignment.Metadata.Name, i, err)
		}

		if err := scopes.StrongValidate(subAssignment.Scope); err != nil {
			return trace.BadParameter("scoped role assignment %q has invalid scope in sub-assignment %d: %v", assignment.Metadata.Name, i, err)
		}

		if !scopes.PolicyAssignmentScope(subAssignment.Scope).IsSubjectToPolicyResourceScope(assignment.Scope) {
			return trace.BadParameter("scoped role assignment %q has sub-assignment %d with scope %q that is not a sub-scope of the assignment's scope %q", assignment.Metadata.Name, i, subAssignment.Scope, assignment.Scope)
		}
	}
...
func commonValidateAssignment(assignment *srpb.ScopedRoleAssignment) error {
	if assignment.Metadata == nil {
		return trace.BadParameter("scoped role assignment is missing metadata")
	}

	if assignment.Metadata.Name == "" {
		return trace.BadParameter("scoped role assignment is missing name")
	}

	if assignment.Kind == "" {
		return trace.BadParameter("scoped role assignment %q is missing kind", assignment.Metadata.Name)
	}

	if assignment.Kind != KindScopedRoleAssignment {
		return trace.BadParameter("scoped role assignment %q has invalid kind %q, expected %q", assignment.Metadata.Name, assignment.Kind, KindScopedRoleAssignment)
	}

	if assignment.SubKind != "" {
		return trace.BadParameter("scoped role assignment %q has unknown sub_kind %q", assignment.Metadata.Name, assignment.SubKind)
	}

	if assignment.Version == "" {
		return trace.BadParameter("scoped role assignment %q is missing version", assignment.Metadata.Name)
	}

	if assignment.Version != types.V1 {
		return trace.BadParameter("scoped role assignment %q has unsupported version %q (expected %q)", assignment.Metadata.Name, assignment.Version, types.V1)
	}

	if assignment.Scope == "" {
		return trace.BadParameter("scoped role assignment %q is missing scope", assignment.Metadata.Name)
	}

	if assignment.Spec == nil {
		return trace.BadParameter("scoped role assignment %q is missing spec", assignment.Metadata.Name)
	}

	if assignment.Spec.User == "" {
		return trace.BadParameter("scoped role assignment %q is missing user", assignment.Metadata.Name)
	}
