...
const (
	...
	scopedRoleUserLockComponent   = "user_lock"
	scopedRoleRoleLockComponent   = "role_lock"
)
...
func (s *ScopedRoleService) CreateScopedRole(ctx context.Context, req *srpb.CreateScopedRoleRequest) (*srpb.CreateScopedRoleResponse, error) {
	...
	lockItem, err := s.bk.Get(ctx, scopedRoleRoleLockKey(role.Metadata.Name))
	if err != nil && !trace.IsNotFound(err) {
		return nil, trace.Wrap(err)
	}

	lockCondition := backend.NotExists()
	lockAction := backend.Nop()
	if lockItem != nil {
		...
		for assignment, err := range s.StreamScopedRoleAssignments(ctx) {
			...
			for _, subAssignment := range assignment.Spec.Assignments {
				if subAssignment.Role != role.Metadata.Name {
					continue
				}
				...
				_, err = s.bk.Get(ctx, scopedRoleKey(role.Metadata.Name))
				if err != nil {
					if trace.IsNotFound(err) {
						// this is a dangling assignment, we need to return an error
						return nil, trace.BadParameter("cannot create scoped role %q while dangling assignment %q references it", role.Metadata.Name, assignment.Metadata.Name)
					}
					return nil, trace.Wrap(err)
				}
				return nil, trace.AlreadyExists("scoped role %q already exists", role.Metadata.Name)
			}
		}
	}
	...
	revision, err := s.bk.AtomicWrite(ctx, []backend.ConditionalAction{
		...
		{
			Key:       scopedRoleRoleLockKey(role.Metadata.Name),
	...
	if err != nil {
		if errors.Is(err, backend.ErrConditionFailed) {
			_, roleErr := s.bk.Get(ctx, item.Key)
			if roleErr != nil && !trace.IsNotFound(roleErr) {
				return nil, trace.Wrap(roleErr)
			}
			if roleErr == nil {
				return nil, trace.AlreadyExists("scoped role %q already exists", role.Metadata.Name)
			}
			return nil, trace.CompareFailed("scoped role %q or associated resourced have been concurrently modified", role.Metadata.Name)
		}
		return nil, trace.Wrap(err)
	}
...
func (s *ScopedRoleService) UpdateScopedRole(ctx context.Context, req *srpb.UpdateScopedRoleRequest) (*srpb.UpdateScopedRoleResponse, error) {
	...
	if role.Metadata.Revision != "" && role.Metadata.Revision != extant.Role.Metadata.Revision {
		return nil, trace.CompareFailed("scoped role %q has been concurrently modified", role.Metadata.Name)
	}
	...
	if scopes.Compare(role.Scope, extant.Role.Scope) != scopes.Equivalent {
		return nil, trace.BadParameter("cannot modify the resource scope of scoped role %q (%q -> %q)", role.Metadata.Name, extant.Role.Scope, role.Scope)
	}
	...
	if lockItem != nil {
		...
		for assignment, err := range s.StreamScopedRoleAssignments(ctx) {
			...
			for _, subAssignment := range assignment.Spec.Assignments {
				...
				if !sr.RoleIsAssignableAtScope(extant.Role, subAssignment.Scope) {
				...
				if !sr.RoleIsAssignableAtScope(role, subAssignment.Scope) {
					return nil, trace.BadParameter("update of scoped role %q would invalidate assignment %q which assigns it to user %q at scope %q", role.Metadata.Name, assignment.Metadata.Name, assignment.Spec.User, subAssignment.Scope)
				}
			}
		}
	}
...
func (s *ScopedRoleService) DeleteScopedRole(ctx context.Context, req *srpb.DeleteScopedRoleRequest) (*srpb.DeleteScopedRoleResponse, error) {
	...
	lockItem, err := s.bk.Get(ctx, scopedRoleRoleLockKey(roleName))
	if err != nil && !trace.IsNotFound(err) {
		return nil, trace.Wrap(err)
	}
	...
	for assignment, err := range s.StreamScopedRoleAssignments(ctx) {
		...
		for _, subAssignment := range assignment.Spec.Assignments {
			if subAssignment.Role == roleName {
				return nil, trace.BadParameter("cannot delete scoped role %q while assignment %q assigns it to a user", roleName, assignment.GetMetadata().GetName())
			}
		}
	}
	...
	_, err = s.bk.AtomicWrite(ctx, []backend.ConditionalAction{
		...
		{
			Key:       scopedRoleRoleLockKey(roleName),
			Condition: lockCondition,
			Action:    backend.Delete(),
		},
	})
	...
	if err != nil {
		if errors.Is(err, backend.ErrConditionFailed) {
			_, roleErr := s.bk.Get(ctx, scopedRoleKey(roleName))
			if roleErr != nil {
				if trace.IsNotFound(roleErr) {
...
func (s *ScopedRoleService) CreateScopedRoleAssignment(ctx context.Context, req *srpb.CreateScopedRoleAssignmentRequest) (*srpb.CreateScopedRoleAssignmentResponse, error) {
	...
	if len(assignment.Spec.Assignments) > sr.MaxRolesPerAssignment {
	...
	condacts := []backend.ConditionalAction{
		...
		{
			Key:       scopedRoleUserLockKey(assignment.Spec.User),
	...
	for _, subAssignment := range assignment.Spec.Assignments {
		...
		roleRevision, ok := req.RoleRevisions[subAssignment.Role]
		if !ok {
			...
			return nil, trace.BadParameter("missing role revision for role %q in backend create (this is a bug)", subAssignment.Role)
		}

		rrsp, err := s.GetScopedRole(ctx, &srpb.GetScopedRoleRequest{
			Name: subAssignment.Role,
		})
		if err != nil {
			return nil, trace.Wrap(err)
		}

		if rrsp.Role.Metadata.Revision != roleRevision {
			return nil, trace.CompareFailed("scoped role %q has been concurrently modified", subAssignment.Role)
		}
		...
		if scopes.Compare(rrsp.Role.Scope, assignment.Scope) != scopes.Equivalent {
			return nil, trace.BadParameter("role %q is not scoped to the same resource scope as assignment %q (%q -> %q)", subAssignment.Role, assignment.Metadata.Name, rrsp.Role.Scope, subAssignment.Scope)
		}

		// verify that the role is assignable at the specified scope
		if !sr.RoleIsAssignableAtScope(rrsp.Role, subAssignment.Scope) {
			return nil, trace.BadParameter("scoped role %q is not configured to be assignable at scope %q", subAssignment.Role, subAssignment.Scope)
		}
		...
		condacts = append(condacts, []backend.ConditionalAction{
			{
				Key:       scopedRoleKey(subAssignment.Role),
				Condition: backend.Revision(roleRevision),
				Action:    backend.Nop(),
			},
			{
				Key:       scopedRoleRoleLockKey(subAssignment.Role),
				Condition: backend.Whatever(),
				Action: backend.Put(backend.Item{
					Value: []byte(uuid.New().String()),
				}),
	...
	if err != nil {
		if errors.Is(err, backend.ErrConditionFailed) {
			if _, err := s.bk.Get(ctx, item.Key); err == nil {
				return nil, trace.AlreadyExists("scoped role assignment %q already exists", assignment.GetMetadata().GetName())
			}

			// check if any of the associated roles have been concurrently modified
			for name, revision := range req.RoleRevisions {
				item, err := s.bk.Get(ctx, scopedRoleKey(name))
				if err != nil {
					if trace.IsNotFound(err) {
						return nil, trace.BadParameter("scoped role assignment %q assigns nonexistent role %q", assignment.GetMetadata().GetName(), name)
					}
					return nil, trace.Wrap(err)
				}
				if item.Revision != revision {
					return nil, trace.CompareFailed("scoped role assignment %q failed due to concurrent modification of associated role %q", assignment.GetMetadata().GetName(), name)
				}
			}
...
func (s *ScopedRoleService) DeleteScopedRoleAssignment(ctx context.Context, req *srpb.DeleteScopedRoleAssignmentRequest) (*srpb.DeleteScopedRoleAssignmentResponse, error) {
	...
	if rev := req.GetRevision(); rev != "" && rev != extant.Assignment.Metadata.Revision {
	...
	userLockItem, err := s.bk.Get(ctx, scopedRoleUserLockKey(extant.Assignment.Spec.User))
	if err != nil && !trace.IsNotFound(err) {
		return nil, trace.Wrap(err)
	}
	...
	if userLockItem != nil {
		...
		for assignment, err := range s.StreamScopedRoleAssignments(ctx) {
			...
			if assignment.Spec.User != extant.Assignment.Spec.User {
				// skip assignments related to other users
				continue
			}
			if assignment.Metadata.Name == extant.Assignment.Metadata.Name {
	...
	condacts := []backend.ConditionalAction{
		{
			...
			Condition: backend.Revision(extant.Assignment.Metadata.Revision),
			Action:    backend.Delete(),
		},
		{
			Key:       scopedRoleUserLockKey(extant.Assignment.Spec.User),
			Condition: userLockCondition,
			Action:    userLockAction,
		},
	}

	for _, subAssignment := range extant.Assignment.Spec.Assignments {
...
func scopedRoleUserLockKey(username string) backend.Key {
	return backend.NewKey(scopedRolePrefix, scopedRoleUserLockComponent, username)
}

func scopedRoleRoleLockKey(roleName string) backend.Key {
	return backend.NewKey(scopedRolePrefix, scopedRoleRoleLockComponent, roleName)
}
...
func scopedRoleToItem(role *srpb.ScopedRole) (backend.Item, error) {
	...
	if role.Metadata.Expires != nil {
		// TODO(fspmarshall): decide on a model for scoped role expiration.
	...
	return backend.Item{
		...
		Expires:  utils.TimeFromProto(role.GetMetadata().GetExpires()),
	}, nil
}
...
func scopedRoleAssignmentToItem(assignment *srpb.ScopedRoleAssignment) (backend.Item, error) {
	...
	if assignment.Metadata.Expires != nil {
		// TODO(fspmarshall): decide on a model for scoped role assignment expiration.
	...
	return backend.Item{
		...
		Expires:  utils.TimeFromProto(assignment.GetMetadata().GetExpires()),
	}, nil
}

// scopedRoleWithRevision creates a shallow copy of the provided role with an updated revision.
func scopedRoleWithRevision(role *srpb.ScopedRole, revision string) *srpb.ScopedRole {
	roleCopy := *role
	metadata := *role.Metadata
	metadata.Revision = revision
	roleCopy.Metadata = &metadata
	return &roleCopy
}
...
func scopedRoleAssignmentWithRevision(assignment *srpb.ScopedRoleAssignment, revision string) *srpb.ScopedRoleAssignment {
	...
	metadata.Revision = revision
	assignmentCopy.Metadata = &metadata
