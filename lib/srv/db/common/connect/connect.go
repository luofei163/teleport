...
func Connect(ctx context.Context, params ConnectParams) (net.Conn, ConnectStats, error) {
	...
	// group the servers by target health such that healthy servers are tried
	// before "less healthy" servers.
	serverGroups := types.GroupByTargetHealth(params.Servers)
	for _, group := range serverGroups {
		params.ShuffleFunc(group)
	}
	...
	for _, server := range slices.Concat(serverGroups...) {
