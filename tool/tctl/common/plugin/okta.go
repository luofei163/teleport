...
func (p *PluginsCommand) initInstallOkta(parent *kingpin.CmdClause) {
	...
	p.install.okta.cmd.
		Flag("assign-okta-requester-role", "If user synchronization is enabled, assign the builtin okta-requester role to synchronized users").
		Default("true").
	...
	p.install.okta.cmd.
		Flag("system-logs-export", "Enables the Teleport Identity Security SIEM integration for Okta.").
		BoolVar(&p.install.okta.enableAuditLogsExport)
}
